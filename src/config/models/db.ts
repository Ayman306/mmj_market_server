...
// Function to execute a query
   query = async (query: string, params?: any[],dbClient?:any) => {
    ...
    public insertSQL(row:any, configSQL:any): any {
      ...
      row = pgp.helpers.insert(row, columnSet, { table: configSQL.table }) + " RETURNING id";
  ...
  public upsertSQL(row:any, configSQL:any): any {
    ...
    row = pgp.helpers.insert(row, columnSet, { schema: configSQL.schema, table: configSQL.table }) + onConflict + " RETURNING id";
...
public updateSQLMultipleKey(row:any, configSQL:any, key?:any): any {
  ...
  if (row[0]) {
      ...
      columnSet = Object.keys(row[0]);
      const whereCLause = key.map((ele:any, i:any) => {
          
          return `v.${ele}=t.${ele}`;

      }).join(' AND ')
      row = pgp.helpers.update(row, columnSet, { schema: configSQL.schema, table: configSQL.table }) + `WHERE ${whereCLause} RETURNING t.id`;
  } else {
      const whereCLause = key.map((ele:any, i:any) => {
      ...
      row.updated_date = pgp.as.date(new Date());
      columnSet = Object.keys(row);
      row = pgp.helpers.update(row, columnSet, { schema: configSQL.schema, table: configSQL.table }) + `where ${whereCLause} RETURNING id`;
  }
